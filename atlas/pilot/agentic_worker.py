...
def build_agentic_prompt(task: AgenticCompanyTask, config: PilotConfig, profile_summary: dict) -> str:
    ...
    return (
        ...
        f"3. If a public ATS/API is resolved, use search_official_career_site + open_official_job_detail "
        f"(fast path). Otherwise use the STATEFUL BROWSER: browser_start(the official career URL), then for "
        f"EACH lane call browser_search_lane(lane, query, 'India'); browser_observe to read handles; "
        f"browser_open_job_detail on the most plausible India roles; browser_back between details. You may "
        f"also use web_search_leads / web_fetch_official to locate an official indexed detail page when the "
        f"UI resists automation (leads are validated in Python; only official/ATS hosts are accepted).\n"
        f"4. Cover ALL FIVE lanes with 1-2 India-scoped query variants each ({locs}).\n"
        f"5. Finally call submit_company_search_result. Always submit, even for a truthful blocker.\n\n"
